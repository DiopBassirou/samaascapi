...
class AllMatchController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            // Charger les relations de façon sécurisée, cacher homme_du_match pour éviter les N+1
            $matches = $query->with(['asc'])->get();
            $matches->each(function ($match) {
                $match->makeHidden('homme_du_match');
            });
            ...
            foreach ($matches as $match) {
                ...
                // Chercher l'adversaire via adversaire_code d'abord
                ...
                if ($match->poule_team_id) {
                    try {
                        ...
                        if ($opponent) {
                            if ($opponent->asc) {
                                $adversaireName = $opponent->asc->nom;
